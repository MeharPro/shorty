...
const SERVERLESS_CACHE_DIR = path.join(os.tmpdir(), 'brainrot');
const isServerlessRuntime = Boolean(
  process.env.VERCEL || process.env.LAMBDA_TASK_ROOT || process.env.AWS_REGION
);

export const BRAINROT_CACHE_DIR = isServerlessRuntime
  ? SERVERLESS_CACHE_DIR
  : path.resolve(process.cwd(), '.cache/brainrot');
...
export function buildGameplayAssetResponse(cloudName, duration = GAMEPLAY_DERIVED_DURATION) {
  return {
...
export async function ensureBrainrotGameplayAsset({ cloudName, apiKey, apiSecret }) {
  ...
  return {
    asset: buildGameplayAssetResponse(
      cloudName,
      Number(upload?.duration || GAMEPLAY_DERIVED_DURATION) || GAMEPLAY_DERIVED_DURATION
    ),
    cached,
  };
}
