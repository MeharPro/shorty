...
function parseTranscriptBlocksFromSegments(transcriptSegments, sourceDurationSeconds) {
    ...
    .map((segment, index) => {
      ...
      const start = Number.isFinite(rawStart)
        ...
        : Number.isFinite(firstWord?.start)
          ? Number(firstWord.start)
          : 0;
      const fallbackEnd = Number.isFinite(rawEnd)
        ? rawEnd
      ...
      return {
        ...
        start,
        end: Math.max(fallbackEnd, start + 0.2),
        text,
      };
    })
    .filter(Boolean);
...
function buildTranscriptWindow(blocks, anchorIndex, sourceDurationSeconds) {
  const minDuration = 30;
  const idealDuration = 38;
  const maxDuration = 60;
  const anchor = blocks[anchorIndex];
  const anchorDuration = Math.max(2, (anchor.end || anchor.start + 2) - anchor.start);
  let startIndex = anchorIndex;
  let endIndex = anchorIndex;
  let startOffset = Math.max(0, anchor.start - Math.min(2.5, anchorDuration * 0.35));
  let endOffset = Math.max(anchor.end || anchor.start + 2, anchor.start + 2);

  while (endIndex + 1 < blocks.length && endOffset - startOffset < minDuration) {
    const next = blocks[endIndex + 1];
    const candidateEnd = Math.max(next.end || next.start + 2, next.start + 2);
    if (candidateEnd - startOffset > maxDuration) {
      break;
    }
    endIndex += 1;
  ...
  while (endIndex + 1 < blocks.length && endOffset - startOffset < idealDuration) {
    const next = blocks[endIndex + 1];
    const candidateEnd = Math.max(next.end || next.start + 2, next.start + 2);
    if (candidateEnd - startOffset > maxDuration) {
      break;
    ...
    if (/[.!?]$/.test(next.text.trim()) && endOffset - startOffset >= minDuration) {
      break;
    }
  }

  while (startIndex > 0 && endOffset - startOffset < minDuration) {
    const previous = blocks[startIndex - 1];
    const candidateStart = Math.max(0, previous.start - 1.5);
    if (endOffset - candidateStart > maxDuration) {
      break;
    }
    startIndex -= 1;
  ...
  if (endOffset - startOffset < minDuration) {
    endOffset = Math.min(sourceDurationSeconds, startOffset + minDuration);
  }

  let duration = endOffset - startOffset;
  if (duration > maxDuration) {
    endOffset = startOffset + maxDuration;
    duration = maxDuration;
  }

  if (endOffset > sourceDurationSeconds) {
    endOffset = sourceDurationSeconds;
    startOffset = Math.max(0, endOffset - Math.min(maxDuration, Math.max(minDuration, duration)));
    duration = endOffset - startOffset;
  }

  duration = clamp(Math.round(duration), minDuration, maxDuration);
  startOffset = clamp(Math.round(startOffset), 0, Math.max(0, sourceDurationSeconds - duration));
  ...
  return {
    ...
    startIndex,
    endIndex,
    transcriptExcerpt: blocks.slice(startIndex, endIndex + 1).map((block) => block.text).join(' '),
  };
}
...
function deriveTranscriptCandidates(blocks, sourceDurationSeconds) {
  return blocks.map((block, index) => {
    const window = buildTranscriptWindow(blocks, index, sourceDurationSeconds);
    const scoring = scoreCandidate(
      window.transcriptExcerpt || block.text,
      window.startOffset,
      window.duration,
    ...
    return {
      ...
      transcriptExcerpt: window.transcriptExcerpt || block.text,
      hook,
      captionLines,
...
export function buildReelPlan({
  ...
  const segmentBlocks = parseTranscriptBlocksFromSegments(transcriptSegments, safeDuration);
  const transcriptBlocks = segmentBlocks.length
    ? segmentBlocks
    : parseTranscriptBlocks(transcriptText, safeDuration);
