...
function applyVisualBoost(candidate, highlights) {
  ...
  return {
    ...candidate,
    viralityScore: clampScore(candidate.viralityScore + boost),
    ...
    focusStrategy: highlight.focusStrategy || (highlight.faceCount >= 4 ? 'group' : 'speaker'),
    cameraMotion: highlight.cameraMotion || (highlight.score >= 0.78 ? 'shake' : highlight.score >= 0.56 ? 'dynamic' : 'steady'),
    scoreBreakdown: {
...
function deriveExpressionCandidates(highlights, sourceDurationSeconds) {
  if (!Array.isArray(highlights) || !highlights.length) {
    return [];
  }
...
export function buildReelPlan({ transcriptText, sourceDurationSeconds, visualAnalysis }) {
  ...
  const transcriptCandidates = deriveTranscriptCandidates(transcriptBlocks, safeDuration).map((candidate) =>
    applyVisualBoost(candidate, highlights)
  );
  const visualCandidates = deriveVisualCandidates(safeDuration);
  const expressionCandidates = deriveExpressionCandidates(highlights, safeDuration);
